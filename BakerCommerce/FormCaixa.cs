...
{
    public partial class BakerCommerce : Form
    {
        Model.Usuario usuario;
        public BakerCommerce(Model.Usuario usuario)
        {
            InitializeComponent();
        ...
        private void cxbPagamento_CheckedChanged(object sender, EventArgs e)
        {
        ...
        private void dgvComanda_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            int ls = dgvComanda.SelectedCells[0].RowIndex;

            // Colocar os ID do produto no  campo de código:
            txbComanda.Text = dgvComanda.Rows[ls].Cells[0].Value.ToString();
            // Colocar nome do produto no campo de informãções:
            txbComanda.Text = dgvComanda.Rows[ls].Cells[1].Value.ToString();
        }
    }
}
